...
import { getIssueById, addComment, addSupport, removeSupport, hasUserSupported } from '../../utils/issues';
...
export default function StudentIssueDetailPage() {
  ...
  const [isSupported, setIsSupported] = useState(() => hasUserSupported(issueId, session?.email));
  ...
  const updatedIssue = useMemo(() => getIssueById(issueId), [issueId, refreshKey]);
  ...
  const handleAddComment = (e) => {
    ...
    const result = addComment(issueId, commentText, session?.email, session?.fullName);
    if (result.ok) {
      setCommentText('');
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-md">
            {updatedIssue?.imageUrl && (
              <div className="relative h-64 w-full overflow-hidden bg-slate-200">
                <img
                  src={updatedIssue.imageUrl}
                  alt={updatedIssue.title}
            ...
            <div className="p-6">
              <div className="mb-4 flex flex-wrap items-start justify-between gap-2">
                <div>
                  <h1 className="text-2xl font-bold text-slate-900">{updatedIssue.title}</h1>
                  <p className="mt-1 text-sm text-slate-500">Issue ID: {updatedIssue.id}</p>
                </div>
                <span className={`rounded-full px-3 py-1 text-sm font-semibold ${getStatusColor(updatedIssue.status)}`}>
                  {getStatusLabel(updatedIssue.status)}
                </span>
              </div>

              <p className="mb-4 text-slate-700">{updatedIssue.description}</p>

              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-lg bg-slate-50 p-3">
                  <p className="text-xs text-slate-600">Reported By</p>
                  <p className="font-semibold text-slate-900">{updatedIssue.studentName}</p>
                  <p className="text-xs text-slate-500">{updatedIssue.studentEmail}</p>
                </div>
                <div className="rounded-lg bg-slate-50 p-3">
                  <p className="text-xs text-slate-600">Category</p>
                  <p className="font-semibold text-slate-900 capitalize">{updatedIssue.category}</p>
                </div>
              </div>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-900">{updatedIssue.location}</p>
                  {updatedIssue.latitude && updatedIssue.longitude && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-900">{formatDate(updatedIssue.createdAt)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4 h-fit">
          ...
          >
            <ThumbsUp size={20} />
            <span>Support ({updatedIssue.supports || 0})</span>
          </button>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-slate-900">{updatedIssue.supports || 0}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-600 flex items-center gap-2">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            ...
            <div className="space-y-2">
              <div className={`rounded text-sm px-3 py-2 text-center font-semibold ${getStatusColor(updatedIssue.status)}`}>
                {getStatusLabel(updatedIssue.status)}
              </div>
            </div>
          </div>
